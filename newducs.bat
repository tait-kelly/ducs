...
echo %TITLE%
echo -----------------------------------------------------------------------------------------
REM echo the version update parameter is %UPDATE%
if NOT "%UPDATE%"=="s" (
...
REM echo I am in the script update section
del Version.txt
curl -LJO  https://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@github.com/tait-kelly/ducs/raw/main/Version.txt > NUL
for /f "tokens=1-2 delims=:" %%a in ('FINDSTR /C:"Version:" Version.txt') do set CURRVER=%%b
echo Version:%VERSION% and there is a version of %CURRVER% online
if %CURRVER%==%VERSION% echo well looks like we have the current version
if %CURRVER% GTR %VERSION% echo looks like there is a newer version
curl -LJo newducs%CURRVER%.bat  https://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@github.com/tait-kelly/ducs/raw/main/newducs.bat
