...
start "ADD SJU Printer"  /wait %CD%\psexec -s \\%TARGET% -u sysadmin -p %pw% c:\Windows\SJU\addprinters.bat >> %CD%\logs\report%TARGET%%today%.txt

rem 10. Remove batch files from machine and current directory
echo %today%%time%: Cleanup time >> %CD%\logs\report%TARGET%%today%.txt
REM echo %today%%time%: Get the system Information >> %CD%\logs\report%TARGET%%today%.txt
REM start "Get System Info"  /wait %CD%\psexec -s \\%TARGET% -u %USERNAME% -p %pw% systeminfo.bat
REM copy x:\sju\%TARGET%sysinfo.txt %cd%\logs\%TARGET%sysinfo.txt
REM echo %today%%time%:I should have a proper sysinfo now.
PAUSE
...
REM del %CD%\Deploy\systeminfo.bat > NUL
